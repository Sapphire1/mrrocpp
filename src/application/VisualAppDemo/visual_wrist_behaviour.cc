...
const int visual_wrist_behaviour::motion_steps_default = 30;
const int visual_wrist_behaviour::motion_steps_min = 10;
const int visual_wrist_behaviour::motion_steps_max = 60;
const double visual_wrist_behaviour::step_time = 0.002;
...
max_angular_speed(0), max_acceleration(0), max_angular_acceleration(0)
{
  char config_section_name[] = { "[object_follower_ib]" };
   ...
   boost::shared_ptr <visual_servo_regulator>  reg_wrist = boost::shared_ptr <visual_servo_regulator> (new regulator_p(ecp_task.config, config_section_name_wrist));
   ...
   boost::shared_ptr <visual_servo> wrist_wrist_vs = boost::shared_ptr <visual_servo> (new ib_eih_visual_servo(reg_wrist, ds, config_section_name, ecp_task.config));
   //this->arm_wrist_vs=arm_wrist_vs;
  ...
  string log_enabled_name = "wrist_vs_log_enabled";
  if (ecp_task.config.exists(log_enabled_name, section_name)
  && ecp_task.config.value <bool> (log_enabled_name, section_name)) {
  unsigned int capacity = ecp_task.config.value <unsigned int> ("wrist_vs_log_capacity", section_name);
  std::string server_addr = ecp_task.config.value <std::string> ("wrist_vs_log_server_addr", section_name);
  int server_port = ecp_task.config.value <int> ("wrist_vs_log_server_port", section_name);
...
bool visual_wrist_behaviour::first_step()
{
...
log_dbg("visual_wrist_behaviour::first_step() end\n");
return true;
}

bool visual_wrist_behaviour::next_step()
{
  std::cout<<"visual_wrist_behaviour::next_step\n";
  ...
  log_dbg("visual_wrist_behaviour::next_step() end\n");
return /*!any_condition_met;*/true;
}
 // next_step()
 void visual_wrist_behaviour::constrain_position(lib::Homog_matrix & new_position)
 {
 ...
 void visual_wrist_behaviour::constrain_vector(Eigen::Matrix <double, 3, 1> &ds, Eigen::Matrix <double, 3, 1> &prev_v, Eigen::Matrix <
...
Eigen::Matrix <double, 3, 1> dv = v - prev_v;
 ...
 void visual_wrist_behaviour::constrain_speed_accel(lib::Homog_matrix & position_change)
 {
...
aa_vector.block(3, 0, 3, 1) = dalpha;
 ...
 const lib::Homog_matrix& visual_wrist_behaviour::get_current_position() const
 {
  return current_position;
 }
 
 void visual_wrist_behaviour::configure(const std::string & sensor_prefix)
 {
 // log_dbg("void visual_servo_manager::configure() 1\n");
  log_dbg("void visual_servo_manager::configure() 2\n");
  int i = 0;
  wrist_vs->get_sensor()->configure_sensor();
  char sensor_suffix[64];
  sprintf(sensor_suffix, "%02d", i);
  lib::sensor::SENSOR_t sensor_id = sensor_prefix + sensor_suffix;
  sensor_m[sensor_id] = wrist_vs->get_sensor().get();

  log_dbg("void visual_wrist_behaviour::configure() 3\n");
 }
 
void visual_wrist_behaviour::add_position_constraint(boost::shared_ptr <position_constraint> new_constraint)
 {
  position_constraints.push_back(new_constraint);
}
 
 double visual_wrist_behaviour::get_linear_speed() const
 {
  return velocity.norm();
 }
 
 double visual_wrist_behaviour::get_angular_speed() const
 {
  return angular_velocity.norm();
 }
 
 double visual_wrist_behaviour::get_linear_acceleration() const
 {
  return acceleration.norm();
 }
 
 double visual_wrist_behaviour::get_angular_acceleration() const
 {
  return angular_acceleration.norm();
 }
 
 const boost::shared_ptr <mrrocpp::ecp::servovision::visual_servo> & visual_wrist_behaviour::get_servo() const
 {
  return wrist_vs;
 }
 
double visual_wrist_behaviour::get_dt() const
 {
  return dt;
 }
 
 void visual_wrist_behaviour::set_new_motion_steps(int new_motion_steps)
 {
 // ss<<"new_motion_steps = "<<new_motion_steps<<"\n";
  ...
  //log_dbg("visual_wrist_behaviour:set_new_motion_steps(): this->new_motion_steps = %d\n", this->new_motion_steps);
 }
 
 int visual_wrist_behaviour::get_new_motion_steps() const
 {
  return new_motion_steps;
 }
 
 int visual_wrist_behaviour::get_motion_steps() const
 {
  return motion_steps;
 }
 
 int visual_wrist_behaviour::get_motion_steps_base() const{
  return motion_steps_base;
 }
 
 void visual_wrist_behaviour::update_motion_steps(ecp_mp::sensor::discode::reading_message_header rmh)
 {
  Types::Mrrocpp_Proxy::Reading* reading = wrist_vs->get_reading();
  ...
  image_mrroc_delay -= wrist_vs->get_sensor()->get_mrroc_discode_time_offset();
 ...
 lib::Homog_matrix visual_wrist_behaviour::get_aggregated_position_change()
 {
  bool reading_received = false;
  ecp_mp::sensor::discode::reading_message_header rmh;
  if (macrostep_length_control && wrist_vs->get_sensor()->get_state()
  == ecp_mp::sensor::discode::discode_sensor::DSS_READING_RECEIVED) {
  rmh = wrist_vs->get_sensor()->get_rmh();
  reading_received = true;
  }
 
  lib::Homog_matrix pc = wrist_vs->get_position_change(get_current_position(), get_dt());
