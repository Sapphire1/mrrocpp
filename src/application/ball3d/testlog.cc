...
int main(int argc, char *argv[])
{
	...
	desc.add_options()
	    ...
	    ("filename_prefix", po::value<string>(&filename_prefix)->default_value("lukaszowepomiary"), "filename_prefix")
	    ("number_of_reconnections", po::value<int>(&number_of_reconnections)->default_value(1), "number_of_reconnections")
	;
	...
	logger_client log(buffer_capacity, server_name.c_str(), server_port, "test;los;rys;kot;sowa", filename_prefix);
	...
	for(int j=0; j<number_of_reconnections; ++j){
		...
		for(int i=0; i<messages_count; ++i){
			...
			sprintf(lm.text, "Message %d %d %d %d", i, i*10, i*20, i*30, i*40);
			log.log(lm);
